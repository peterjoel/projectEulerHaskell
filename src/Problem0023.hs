...
calc = sum $ filter (not . abundantSum) [1..28123]

abundantSum n     = any (isAbundant . (n-)) [i| i <- abundantNumsTo (n `div` 2)]

abundantNumsTo n  = takeWhile (<= n) $ filter isAbundant' [1..]

isAbundant' n     = (sum . divisors) n > 2 * n
 
isAbundant n      = Set.member n abundantNums

abundantNums      = Set.fromList $ abundantNumsTo 28123
